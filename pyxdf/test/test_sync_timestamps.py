...
def streams():
    ...
    class MockStream(dict):
        def __init__(
            self, timestamps, timeseries, effective_srate, channel_format
        ):
    ...
    streams[1] = MockStream(
        np.linspace(1, 2, 1001), np.linspace(1, 2, 1001), 1000, "float32"
    )

    # slowest stream, earliest timestamp
    streams[2] = MockStream(
        np.linspace(0.1, 1.1, 251), np.linspace(2, 1, 251), 250, "float32"
    )

    # marker stream
    streams[3] = MockStream(
        [0.2, 1.1071, 1.2, 1.9, 2.5],
        ["mark_" + str(n) for n in range(0, 5, 1)],
        0,
        "string",
    )
    # integer datatype stream,
    streams[4] = MockStream(
        np.linspace(0.1, 1.1, 251),
...
def test_timestamps(synced):
    ...
    for s in synced.values():
        assert np.all(
            np.isclose(s.time_stamps, np.linspace(0.1, 2.5, 2401))
        )
...
def test_markerstream(synced, streams):
    ...
    assert np.all(
        np.equal(
            synced[3].time_stamps[idx],
            np.array(
                [0.2, 1.107, 1.2, 1.9, 2.5]  # identical  # shifted  # shifted
            ),
        )
    )
...
def test_interpolation(synced):
    ...
    assert np.all(
        np.isclose(synced[2].time_series[idx], np.linspace(2, 1, 1001))
    )
...
def test_extrapolation(synced, streams):
    ...
    expectation = np.hstack(
        (
            np.linspace(0.1, 1, 900, endpoint=False),
    ...
    expectation = synced[3].time_series.shape[0] - len(
        streams[3].time_series
    )
...
def test_identity_after_interpolation(synced, streams):
    ...
    idx = np.where(~np.isnan(synced[1].time_series))[0]
    assert np.all(
        np.isclose(synced[1].time_series[idx], streams[1].time_series)
    )
